...
// PostConsensusProcessingDIYBaseline is called by consensus participants, after consensus is done, to:
// 1. add the new block to blockchain
// 3. send cross shard tx receipts to destination shard
func (node *Node) PostConsensusProcessingDIYBaseline(newBlock *types.Block) error {
...
// PostConsensusProcessingDIYLattice is called by consensus participants, after consensus is done, to:
// 1. add the new block to blockchain
// 3. send cross shard tx receipts to destination shard
func (node *Node) PostConsensusProcessingDIYLattice(newBlock *types.Block) error {
	if node.Consensus.IsLeader() {
		// if node.NodeConfig.ShardID == shard.BeaconChainShardID {
		// 	node.BroadcastNewBlock(newBlock)
		// }
